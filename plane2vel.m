...
v1(1) = input('Enter the initial velocity value of joint-1: ');

for i = 2:n+1
    v1(i) = input('Enter the via point velocity of joint-1: ');
end

v1(n+2) = input('Enter the final velocity values of joint-1: ');

v2(1) = input('Enter the initial velocity value of joint-2: ');

for i = 2:n+1
    v2(i) = input('Enter the via point velocity of joint-2: ');
end

v2(n+2) = input('Enter the final velocity values of joint-2: ');

for i = 1:n+2
    theta2_i = rad2deg(2 * atan2(sqrt(abs((l1 + l2)^2 - (xvalue(i)^2 + yvalue(i)^2))), sqrt(abs(xvalue(i)^2 + yvalue(i)^2 - (l1 - l2)^2))));
...
subplot(4,2,5)
plot(linspace(0, tt, length(vel1)), vel1)
title('velocity 1')

subplot(4,2,6)
plot(linspace(0, tt, length(vel2)), vel2)
title('velocity 2')

subplot(4,2,7)
plot(linspace(0, tt, length(acc1)), acc1)
title('acceleration 1')

subplot(4,2,8)
plot(linspace(0, tt, length(acc2)), acc2)
